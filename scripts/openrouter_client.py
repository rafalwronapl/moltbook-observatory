...
# Load API key - simplified (no OpenClaw dependency)
OPENROUTER_API_KEY = None

# Source 1: Environment variable
OPENROUTER_API_KEY = os.environ.get("OPENROUTER_API_KEY")

# Source 2: Project config/.env file
if not OPENROUTER_API_KEY:
    PROJECT_ENV = Path(__file__).parent.parent / "config" / ".env"
...
def call_kimi(prompt: str, system_prompt: str = None, model: str = DEFAULT_MODEL,
    ...
    for attempt in range(max_retries):
        try:
            ...
            # (Kimi K2.5 sometimes puts response in reasoning field)
