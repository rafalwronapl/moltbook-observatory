...
# Base directory (C:\moltbook-scripts)
BASE_DIR = Path(os.environ.get("OBSERVATORY_HOME", Path(__file__).parent))
PROJECT_ROOT = BASE_DIR  # Alias for compatibility

# Config directory (for .env, twitter_keys.json, etc.)
...
# Website (moltbook-observatory React app - separate project)
WEBSITE_DIR = Path("C:/moltbook-observatory")
...
def load_api_keys() -> dict:
    ...
    # Load Twitter keys from dedicated file
    twitter_keys_path = CONFIG_DIR / "twitter_keys.json"
    if twitter_keys_path.exists():
        try:
            with open(twitter_keys_path, "r") as f:
                twitter_keys = json.load(f)
                keys["TWITTER_API_KEY"] = twitter_keys.get("consumer_key")
                keys["TWITTER_API_SECRET"] = twitter_keys.get("consumer_secret")
                keys["TWITTER_ACCESS_TOKEN"] = twitter_keys.get("access_token")
                keys["TWITTER_ACCESS_SECRET"] = twitter_keys.get("access_token_secret")
                keys["TWITTER_BEARER_TOKEN"] = twitter_keys.get("bearer_token")
        except Exception as e:
            print(f"Warning: Could not load Twitter keys: {e}")

    # Load OpenRouter key from OpenClaw auth-profiles.json
            ...
            try:
                ...
                for profile_name, profile_data in profiles.get("profiles", {}).items():
                    if profile_data.get("provider") == "openrouter" and profile_data.get("key"):
                        keys["OPENROUTER_API_KEY"] = profile_data["key"]
                        break
